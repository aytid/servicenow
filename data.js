...
© 2026 Rohan Aditya`
        ...
        {
    id: '1014',
    title: 'Teams Provider Notifications with Agent-Defined Content',
    category: 'Tutorial',
    excerpt: 'Customize ServiceNow Teams Provider Notifications to allow agents to send personalized Microsoft Teams updates directly from a Change record.',
    tags: ['Microsoft Teams', 'Provider Notification', 'Workspace', 'Change Management', 'GlideAjax'],
    date: '2026-08-05',
    views: 185,
    created: '2026-08-05T12:45:00Z',
    content: `

**Author: Rohan Aditya**

### Key Points Covered
- Microsoft Teams Provider Notifications
- Workspace UI Actions
- GlideAjax
- Event-Driven Notifications
- Dynamic Notification Content

By default, ServiceNow Provider Notifications send predefined messages such as:

> **"The state of this Request has changed."**

While this works for many scenarios, it doesn't give support engineers the flexibility to communicate meaningful, real-time updates.

This customization allows agents to compose their own Teams notification directly from the Change record without modifying notification templates or changing the Change state.

---

## The Problem

Standard Provider Notifications rely on static notification templates.

During Change implementations, engineers often need to communicate updates such as:

- Deployment has started.
- Validation is in progress.
- Maintenance is taking longer than expected.
- Rollback has begun.
- Services have been restored.

Creating a separate notification for each situation isn't practical.

---

## The Solution

A custom **Notify TechOps** button was added to the Change Workspace.

Instead of sending a fixed notification, clicking the button opens a popup where the agent can enter the exact message they want to send.
...
The custom Workspace UI Action displays a modal dialog using **g_modal.showFields()**.

The popup contains a mandatory textarea where the support engineer can enter the notification message.

After clicking **Send**, the Workspace Client Script sends the entered text to the server using **GlideAjax**.
...
function onClick(g_form) {
    ...
    g_modal.showFields({
        ...
        instruction: 'Enter text for notification. This will send an interim update without needing to change the status to Review.',
        fields: [{
            type: 'textarea',
        ...
        var notificationText = result.updatedFields[0].value;

        var ga = new GlideAjax('AFCChangeUtils');
        ga.addParam('sysparm_name', 'notifyTechOps');
        ga.addParam('sysparm_sys_id', g_form.getUniqueValue());
        ga.addParam('sysparm_notification_text', notificationText);

        ga.getXMLAnswer(function(answer) {
            if (answer === 'success')
                g_form.reload();
            else
                g_form.addErrorMessage('Failed to send TechOps notification.');
        });
...
<div class="blog-image">
<img src="images/b14img2.png" alt="Notification Text Popup" />
</div>
...
The Script Include receives the notification text and the Change record Sys ID from the Workspace Client Script.

After validating the Change record, it queues a custom event using **gs.eventQueue()**.

The notification text entered by the agent is passed as **event.parm1**, allowing it to be accessed later by the Provider Notification.

\`\`\`javascript
notifyTechOps: function() {

    var sysId = this.getParameter('sysparm_sys_id');
    var notificationText = this.getParameter('sysparm_notification_text');
    ...
    gs.eventQueue(
        'afc.change.techops.notification',
        gr,
        notificationText,
        gs.getProperty('group.sys.id')
    );
...
## Custom Provider Notification

A custom Provider Notification was created to listen for the event:

> **afc.change.techops.notification**

Unlike standard notifications, this event isn't triggered by a Change state transition. Instead, it is raised manually from the Script Include whenever an agent submits a notification.

This keeps the implementation independent of the Change lifecycle while still using ServiceNow's native Provider Notification framework.

<div class="blog-image">
<img src="images/b14img3.png" alt="Provider Notification Configuration" />
</div>
...
The associated VA Notification Content uses a server-side script to build the notification dynamically.

Instead of displaying predefined text, it reads the value stored in **event.parm1**, which contains the message entered by the support engineer.
...
(function getNotificationHeadingAndMessage(
    ...
    var message = '**Update:** ' + event.parm1;

    return {
        messageHeading: 'Interim update on Change ' + target.number,
...
This allows every Teams notification to contain completely different content while continuing to use the same Provider Notification.

---

## Microsoft Teams Output

Once the event is processed, Microsoft Teams receives a rich notification containing the message entered by the agent.

<div class="blog-image">
<img src="images/b14img4.png" alt="Microsoft Teams Notification" />
</div>
...
- Send personalized Teams notifications
- No hardcoded notification messages
- Uses the standard Provider Notification framework
- No additional notification templates required
- Real-time operational updates
- Improved communication during Change implementations
- Simple and intuitive experience for support engineers
...
Instead of receiving a generic notification like:

> **"The state of this Request has changed."**

Teams users receive meaningful updates written directly by the support engineer, providing much clearer communication throughout the implementation process.
...
### Key Points Covered
- Microsoft Teams Collaboration Chat customization
...
© 2026 Rohan Aditya`
    ...
    category: 'Tutorial',
    excerpt: 'Make the Group By option visible by default in a ServiceNow Data Visualization.',
...
- Updating the visualization JSON configuration
- Avoiding the need to select "Show change group by" manually

---

## How to Make the Group By Option Visible by Default

By default, the **Group By** option in a ServiceNow Data Visualization may be hidden.

Users may need to open the **More actions** menu and select:

**Show change group by**

If you want the **Group By** option to be visible automatically whenever the visualization is opened, you can update the visualization JSON configuration.

---

## Step 1: Open the Visualization JSON

Open the JSON configuration of the required Data Visualization.
...
Save the visualization after making the change.

Once the visualization is loaded again, the **Group By** option will be displayed automatically.

You will no longer need to open the **More actions** menu and select:

**Show change group by**

Instead, the Group By option will already be visible.
...
the **Group By** option will always be displayed by default when the Data Visualization loads.

This is useful when users frequently need to change or view the Group By field and you want the option immediately available without requiring an additional click through the More actions menu.
